...
from opentelemetry.metrics import (
    Meter, Instrument, Counter, UpDownCounter, Gauge, CallbackOptions, Observation, CallbackT
)
...
from typing import Any, cast, Sequence, Iterable, Generator, Optional
...
class OpenTelemetryMetrics:
    ...
    def _counter(
        ...
        if sanitized_name not in self._metrics:
            self._metrics[sanitized_name] = self._meter.create_counter(metric_name, unit, description)
    ...
    def _up_down_counter(
        ...
        sanitized_name = self._sanitize_metric_name(metric_name)
        if sanitized_name not in self._metrics:
            self._metrics[sanitized_name] = self._meter.create_up_down_counter(metric_name, unit, description)
    ...
    def _gauge(
        ...
        sanitized_name = self._sanitize_metric_name(metric_name)
        if sanitized_name not in self._metrics:
            self._metrics[sanitized_name] = self._meter.create_gauge(metric_name, unit, description)
    ...
    def _handle_cover(self, state: State) -> None:
        ...
        for cover_state in cover_states:
            metric.labels(**dict(self._default_attributes(state), state=cover_state)).set(
        ...
        if position is not None:
            ...
            position_metric.labels(**self._default_attributes(state)).set(float(position))
        ...
        if tilt_position is not None:
            ...
            tilt_position_metric.labels(**self._default_attributes(state)).set(float(tilt_position))
    ...
    def _handle_climate(self, state: State) -> None:
        ...
        if current_action := state.attributes.get(ATTR_HVAC_ACTION):
            ...
            for action in HVACAction:
                metric.labels(**dict(self._default_attributes(state), action=action.value)).set(
        ...
        if preset_mode and available_preset_modes:
            ...
            for mode in available_preset_modes:
                preset_metric.labels(**dict(self._default_attributes(state), mode=mode)).set(
        ...
        if fan_mode and available_fan_modes:
            ...
            for mode in available_fan_modes:
                fan_mode_metric.labels(**dict(self._default_attributes(state), mode=mode)).set(
                    float(mode == fan_mode)
                )
    ...
    def _handle_humidifier(self, state: State) -> None:
        ...
        if humidifier_target_humidity_percent:
            ...
            metric.labels(**self._default_attributes(state)).set(humidifier_target_humidity_percent)
    ...
    def _handle_fan(self, state: State) -> None:
        ...
        if fan_speed_percent is not None:
            ...
            fan_speed_metric.labels(**self._default_attributes(state)).set(float(fan_speed_percent))
        ...
        if fan_preset_mode and available_modes:
            ...
            for mode in available_modes:
                fan_preset_metric.labels(**dict(self._default_attributes(state), mode=mode)).set(
    ...
    def _handle_counter(self, state: State) -> None:
        ...
        metric.labels(**self._default_attributes(state)).set(self.state_as_number(state))
    ...
    def _handle_alarm_control_panel(self, state: State) -> None:
        ...
        if current_state:
            ...
            for alarm_state in alarm_states:
                metric.labels(**dict(self._default_attributes(state), state=alarm_state)).set(
